...
```{r}
variables <- names(data)[-2] grupos <- split(variables, ceiling(seq_along(variables) / 2))  for (i in seq_along(grupos)) {   par(mfrow = c(1, 1)) 
```
...
```{r}
normalize <- function(x) { return((x - min(x)) / (max(x) - min(x))) } data_norm <- as.data.frame(lapply(data[,-2],normalize))
```
...
```{r}
data_norm$M <- ifelse(data$V2 == "M", TRUE, FALSE) data_norm$B <- ifelse(data$V2 == "B", TRUE, FALSE)
```
...
for (i in 1:3) {   
  ...
  boxplot(data_norm[, col_start:col_end], main = 'Datos escalados 0,1', col = 'lightgreen', cex.axis = 0.4)   abline(h = 0.5, lwd = 2) 
}
```
...
```{r}
set.seed(1234) n_train <- floor(2/3 * nrow(data_norm))  train <- sample(nrow(data_norm), n_train) 
